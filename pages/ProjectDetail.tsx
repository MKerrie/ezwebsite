  ...
  if (!project) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-white dark:bg-[#020202] text-slate-900 dark:text-white">
        ...
        <Link to="/" className="text-blue-600 underline">Terug naar home</Link>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <div className="fixed top-6 left-6 z-50">
        <button
          onClick={() => navigate(-1)}
          className="flex items-center gap-2 px-4 py-2 rounded-full bg-white/80 dark:bg-white/10 backdrop-blur-xl border border-slate-200 dark:border-white/10 text-sm font-bold uppercase tracking-widest hover:bg-blue-600 hover:text-white hover:border-blue-600 transition-all"
        >
        ...
        <div className="flex gap-3 px-6 md:px-16 pt-6 overflow-x-auto scrollbar-none">
          {images.map((img, i) => (
            <button
              key={i}
              onClick={() => setActiveImg(i)}
              className={`shrink-0 w-20 h-14 md:w-28 md:h-18 rounded-xl overflow-hidden border-2 transition-all ${i === activeImg ? 'border-blue-500' : 'border-transparent opacity-50 hover:opacity-80'}`}
            >
      ...
      <div className="container mx-auto px-6 md:px-16 py-12 md:py-16">
        <div className="grid md:grid-cols-[1fr_340px] gap-12 md:gap-20">
          ...
          <div>
            <div className="flex items-center gap-3 mb-6">
              <span className="px-3 py-1 rounded-full bg-blue-600/10 border border-blue-600/30 text-[9px] font-bold uppercase tracking-[0.3em] text-blue-500">
                {project.category}
              </span>
            ...
            <div className="flex flex-wrap gap-2 mb-12">
              {project.tags.map(tag => (
                <span
                  key={tag}
                  className="px-4 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-widest bg-blue-600/10 border border-blue-600/30 text-blue-500"
                >
                  {tag}
                </span>
              ))}
            ...
            <div className="sticky top-24 space-y-8">
              ...
              <div className="border-t border-slate-200 dark:border-white/10 pt-8 space-y-4">
                {project.url && (
                  ...
                  >
                    <span className="font-display font-bold text-2xl uppercase tracking-tighter group-hover:text-blue-600 transition-colors">
                      Bekijk site
                    </span>
                    <div className="w-10 h-10 rounded-full bg-blue-600 flex items-center justify-center text-white group-hover:scale-110 transition-transform">
                ...
                >
                  ...
                  <div className="w-10 h-10 rounded-full border-2 border-slate-900 dark:border-white flex items-center justify-center group-hover:bg-blue-600 group-hover:border-blue-600 group-hover:text-white transition-all">
                    <ArrowUpRight className="w-5 h-5" />
                  </div>
                </Link>
              </div>
        ...
        <div className="border-t border-slate-200 dark:border-white/10 mt-16 pt-12 grid grid-cols-2 gap-6">
          ...
          >
            ...
            <h3 className="font-display font-bold text-xl uppercase tracking-tighter group-hover:text-blue-600 transition-colors flex items-center gap-2">
